...
# Comfort link
ln -s ~/.zsh_plugins.txt ~/.shlib/.zsh_plugins.txt
```
...
function prompt_lionheart_reco() {
  # Checks whether a reco.sh is in the current directory
  local reco_file="$PWD/reco.sh"
  
  # Alternatively: Check globally for the ~/reco.sh generated by Lionheart
  # local reco_file="$HOME/reco.sh"

  # If the file exists, the segment is rendered
  if [[ -f "$reco_file" ]]; then
    # -f 196 (red), -i (Icon), -t (Text)
    p10k segment -f 196 -i '🚨' -t 'reco.sh'
  fi
}
